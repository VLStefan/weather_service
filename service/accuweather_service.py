...
class AccuWeatherService(ExternalService):
    ...
    async def city_search(self, city: str) -> Dict[str, Any]:
        endpoint = f"http://dataservice.accuweather.com/locations/v1/cities/search?apikey={ACCUWEATHER_TOKEN}&q=city{city}"
        response = await self._request(method="GET", endpoint=endpoint)
        return response

    async def fetch_top_cities(self) -> List[Dict[str, Any]]:
        endpoint = f"http://dataservice.accuweather.com/locations/v1/topcities/150?apikey={ACCUWEATHER_TOKEN}"
        response = await self._request(method="GET", endpoint=endpoint)
        return response

    async def fetch_5_days_forecast(self, city_id: str) -> Dict[str, Any]:
