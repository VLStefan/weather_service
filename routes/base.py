...
from provider.data_provider import (
    ...
    get_weather_forecast_statistics_for_city,
)
from response_models import accuweather, weatherstack

base_router: APIRouter = APIRouter(tags=["weather forecast"], prefix="/forecast")
...
@base_router.get(
    ...
    summary="Get statistics of weather for selected date range.",
)
async def weather_statistics(
    ...
    response = await get_weather_forecast_statistics_for_city(
        city_name=city_name,
        day=day,
        month=month,
        min_year=start_year,
